...
namespace File_Split_and_Join
{
    ...
    public class SplitAndJoinFile
    {
        ...
        public void ExtractData(FinalSplitWin _win, long PointStart, long PointEnd, string destFolder, string newFileName)
        {
        ...
        public void SplitFile(FinalSplitWin _win, string destFolder, long size, bool repeat = false, int repeatEvery = 1)
        {
            int partFile = 0;
            ...
            {
                partFile++;
                ExtractData(_win, StartPosition, StartPosition + size * repeatEvery, destFolder, strNewFileNames + ".part_" + partFile.ToString());
        ...
        public void SplitFile(FinalSplitWin _win, string destFolder, byte[] dataSearch, bool repeat = false, int repeatEvery = 1)
        {
            int partFile = 0;
            long StartPosition = 0;
            ...
            long EndPosition = fs_in.Length;
            string strNewFileNames = Path.GetFileName(this.fs_in.Name);
            long entryPoint = FindData(dataSearch, 0, null);
            ...
            while (entryPoint + dataSearchLength < EndPosition)
            {
                ...
                {
                    countEvery--;
                    entryPoint = FindData(dataSearch, entryPoint + 1, null);
                ...
                partFile++;
                ExtractData(_win, StartPosition, entryPoint, destFolder, strNewFileNames + ".part_" + partFile.ToString());
                countEvery = repeatEvery;
                StartPosition = entryPoint;
            }
            if (StartPosition < entryPoint)
            {
                ExtractData(_win, StartPosition, entryPoint, destFolder, strNewFileNames + ".part_" + (partFile + 1).ToString());
            }
        }
